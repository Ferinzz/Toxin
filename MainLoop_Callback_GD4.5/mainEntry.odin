...
MainLoopStartupCallback :: proc "c" () {
    ...
    fmt.println("OMFG THERE'S A LOOP CALBACK")
...
MainLoopFrameCallback :: proc "c" () {
    ...
    if !callOnce {
        ...
        //GDW.StringConstruct.stringNameNewString(&signalis_SN, string(signalis_CString))
        object:= GDW.gdAPI.classDBConstructObj(&signalis_SN)
        //A scene is not added when running editor mode. Check for the scene before trying to add the child to it.
        //Alternatively you can add a node directly to the root.
        if scene != nil {
        //Add the class to the root of the sceneTree
            GDW.addChild(scene, &object)
        }
        callOnce = true
...
/* Registered after the first frame.
Called for every frame iteration of the main loop. */
MainLoopFrameCallback2 :: proc "c" () {
    context = runtime.default_context()
    //fmt.println("WTAF MAINLOOP TICKS?!")
    GDW.Print.WarningWithMessage("", "in a new callback.", "main", "main", 181, false)
}
...
//Setup a different set of callbacks.
myMainLoopCallbacks2: GDE.MainLoopCallbacks = {
	/* Will be called after Godot is started and is fully initialized. */
	startup_func = MainLoopStartupCallback,
	/* Will be called before Godot is shutdown when it is still fully initialized. */
	shutdown_func = MainLoopShutdownCallback,
	 ...
	 * This is intended to be the equivalent of `ScriptLanguage::frame()` for GDExtension language bindings that don't use the script API.
	 */
	frame_func = MainLoopFrameCallback2,
};
...
extensionInit :: proc "c" (userdata: rawptr, initLevel: GDE.InitializationLevel) {
    ...
    switch initLevel{
        ...
        case .INITIALIZATION_SCENE:
            ...
            */
            signalis_Register(signalis, initLevel)
