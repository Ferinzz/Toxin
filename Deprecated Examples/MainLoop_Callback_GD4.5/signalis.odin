...
signalisBindMethod :: proc "c" (){

    //This function does a lot. I recommend looking at it to understand the steps needed to register a class's function.
    GDW.bindMethod(&signalis_SN, "Some_method_name", somePublicFunction, {GDE.ClassMethodFlags.NORMAL}, "arg1")
...
signalis_ready :: proc "c" (self: ^signalis) {
    ...
    GDW.Print.WarningWithMessage("signalis ready", "I'm ready for anything!", "signalis", "signalis", 222, false)

}

physics_run_once:bool=false
process_run_once:bool=false

signalis_physics :: proc "c" (self: ^signalis, delta: f64) {
    context = runtime.default_context()
    
    if !physics_run_once {
        GDW.Print.WarningWithMessage("signalis physics frame", "Look mah I'm in a game", "signalis", "signalis", 222, false)
    }
    physics_run_once = true
}
...
signalis_process :: proc "c" (self: ^signalis, delta: f64) {
    ...
    if !process_run_once {
        GDW.Print.WarningWithMessage("signalis process", "I'm thinking of what to do. Hm.", "signalis", "signalis", 222, false)
    }
    process_run_once = true
}
